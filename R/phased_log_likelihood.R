...
#' @description Calculates the log likelihood of observing the phased data, given the population size, initial heterozygosity and time since admixture
...
phased_log_likelihood <- function(local_anc_matrix,
  ...
  if(sum( labels %in% 1:4) != length(labels)) {
    stop("local ancestry labels should be [1, 2, 3, 4] for homozygous P, homozygous Q, heterozygous PQ and heterozygous QP \n")
  }
