...
#' @keywords internal
get_cond_prob_vector_phased_inf <- function(info_vector,
                                            freq_ancestor_1,
                                            local_time,
                                            condition = TRUE) {
  di <- info_vector[[1]]
  ...
  seven_states <- single_state_inf(local_time, d = di)
...
#' estimates the time since admixture, given unphased ancestry data and assuming
...
#' should be [0, 1], where 0 indicates the first ancestor, and 1 indicates the
#' second ancestor. Alternatively, the user can provide a vector using labels
#' indicating whether at the specific marker, the focal individual is homozygous
#' for the first ancestor (label = 1), homozygous for the second
#' ancestor (label = 2), heterozygous [0,1] (label = 3) or heterozygous [1, 0]
#' (label = 4).
#' @param locations locations of the used markers (in Morgan)
#' @param freq_ancestor_1 Frequency of ancestor 1 at t = 0
...
#' @param verbose display intermediate output? Default = FALSE
#' @export
estimate_time_phased_inf <- function(local_anc_matrix,
                                     locations,
                                     freq_ancestor_1,
                                     lower_lim = 2,
                                     upper_lim = 1000,
  ...
  calc_ll <- function(params) {
    ...
    local_probs <- apply(to_analyze, 1, get_cond_prob_vector_phased_inf,
                         freq_ancestor_1,
                         local_time = params[[1]],
                         condition = TRUE)

    local_probs[1] <- get_cond_prob_vector_phased_inf(to_analyze[1, ],
                                                      freq_ancestor_1,
                                                      local_time = params[[1]],
                                                      condition = FALSE)
